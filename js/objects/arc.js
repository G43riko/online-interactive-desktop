...
Arc.prototype.clickIn = function(x, y){
	...
	if(new GVector2f(x, y).dist(pos.x + (this.size.x >> 1), pos.y) < SELECTOR_SIZE)
		...
		this.moveType = 1;
	else if(new GVector2f(x, y).dist(pos.x +(this.size.x >> 1), pos.y + this.size.y) < SELECTOR_SIZE)
		this.moveType = 2;
	else if(new GVector2f(x, y).dist(pos.x, pos.y + (this.size.y >> 1)) < SELECTOR_SIZE)
		this.moveType = 3;
...
Arc.prototype.updateCreatingPosition = function(pos){
...
Arc.prototype.draw = function(){
	...
	context.ellipse(this.position.x + (this.size.x >> 1),
					this.position.y + (this.size.y >> 1),
					Math.abs(this.size.x >> 1),
